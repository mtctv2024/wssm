  ...
  async function startProcessing() {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
          drawOnCanvasForProcessing(tempCanvas, baseImage, async function (canvas) {
            try {
              ...
              await saveBlob(blob, `${fileName}_processed.jpg`);
              resolve();
            } catch (error) {
              reject(error);
            }
      ...
      if (postProcessMessage) {
        ...
        postProcessMessage.textContent = isMobileDevice()
          ? 'تم تجهيز الصورة. على الجوال استخدم نافذة المشاركة ثم اختر حفظ الصورة أو حفظ في الملفات.'
          : 'تم تحميل الصور بنجاح. ماذا تريد أن تفعل الآن؟';
      }
    } catch (error) {
      console.error(error);
      if (progressContainer) progressContainer.style.display = 'none';
      if (postProcessMessage) {
        postProcessMessage.style.display = 'block';
        postProcessMessage.textContent = 'حدثت مشكلة أثناء حفظ الصور على هذا الجهاز.';
      }
      alert('تعذر حفظ الصورة على هذا الجهاز. جرّب المشاركة ثم الحفظ في الصور أو الملفات.');
    } finally {
