document.addEventListener('DOMContentLoaded', function () {
  ...
  function addResultItem({ fileName, url, mode, previewUrl }) {
  ...
  async function startProcessing() {
    ...
    try {
      ...
      if (postProcessMessage) {
        ...
        if (isIOS()) {
          postProcessMessage.textContent = 'على الآيفون والآيباد: استخدم زر "فتح الصورة"، ثم احفظها من المتصفح.';
        } else if (isAndroid()) {
          postProcessMessage.textContent = 'على أندرويد: تمت محاولة تنزيل الصور تلقائيًا بالطريقة القديمة، ويمكنك أيضًا استخدام زر "تنزيل الصورة" إذا لزم الأمر.';
        } else {
