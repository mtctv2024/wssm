document.addEventListener('DOMContentLoaded', function () {
  ...
  function addResultItem(fileName, objectUrl, mode) {
    ...
    try {
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        await new Promise((resolve, reject) => {
          drawOnCanvasForProcessing(tempCanvas, baseImage, async function (canvas) {
            try {
              ...
              if (isIOS()) {
                addResultItem(fileName, objectUrl, 'ios-open');
              } else if (isAndroid()) {
      ...
      if (postProcessMessage) {
        ...
        if (isIOS()) {
          postProcessMessage.textContent = 'على الآيفون والآيباد: اضغط "فتح الصورة"، ثم احفظها من المتصفح.';
        } else if (isAndroid()) {
          postProcessMessage.textContent = 'على أندرويد: تمت محاولة تنزيل الصور تلقائيًا، ويمكنك أيضًا استخدام أزرار التنزيل أدناه إذا لزم الأمر.';
        } else {
          postProcessMessage.textContent = 'تم تجهيز الصور بنجاح، ويمكنك تنزيلها من القائمة أدناه.';
        }
      }
    } catch (error) {
      console.error(error);
      ...
      if (postProcessMessage) {
        postProcessMessage.style.display = 'block';
        postProcessMessage.textContent = 'حدثت مشكلة أثناء تجهيز الصور.';
      }
