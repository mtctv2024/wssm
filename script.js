document.addEventListener('DOMContentLoaded', () => {
  ...
  const infoOverlayBox = document.getElementById('infoOverlay');
  ...
  function setInfoOverlayState(open) {
    if (!infoOverlayBox || !infoToggle) return;
    infoOverlayBox.classList.toggle('is-open', open);
    infoToggle.setAttribute('aria-expanded', open ? 'true' : 'false');
  }

  if (infoToggle && infoOverlayBox) {
    infoToggle.addEventListener('click', () => {
      const isOpen = infoOverlayBox.classList.contains('is-open');
      setInfoOverlayState(!isOpen);
    });
  }

  if (infoClose) {
    infoClose.addEventListener('click', () => {
      setInfoOverlayState(false);
    });
  }
