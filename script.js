document.addEventListener('DOMContentLoaded', function () {
  ...
  const maxOutputSizeSlider = document.getElementById('maxOutputSize');
  ...
  function updateValueLabels() {
    ...
    document.getElementById('maxOutputSizeValue').textContent = maxOutputSizeSlider.value;
  }
  ...
  function drawPreviewComposition(ctx, drawX, drawY, drawWidth, drawHeight) {
    if (preloadedTemplate) {
      ctx.globalAlpha = templateOpacitySlider.value / 100;
      ctx.drawImage(preloadedTemplate, drawX, drawY, drawWidth, drawHeight);
      ctx.globalAlpha = 1;
    }
    ...
    if (preloadedLogo) {
      ...
      const positions = {
        'Top-Left': [drawX + margin, drawY + margin],
        'Top-Right': [drawX + drawWidth - newW - margin, drawY + margin],
        'Bottom-Left': [drawX + margin, drawY + drawHeight - newH - margin],
        'Bottom-Right': [drawX + drawWidth - newW - margin, drawY + drawHeight - newH - margin],
        'Center': [drawX + (drawWidth - newW) / 2, drawY + (drawHeight - newH) / 2]
      };
  ...
  window.updateLivePreview = function () {
    ...
    const drawWidth = Math.floor(targetWidth * scale);
    const drawHeight = Math.floor(targetHeight * scale);
    const x = Math.floor((containerWidth - drawWidth) / 2);
    const y = Math.floor((containerHeight - drawHeight) / 2);

    ctx.clearRect(0, 0, containerWidth, containerHeight);
    ctx.drawImage(preloadedBaseImage, x, y, drawWidth, drawHeight);
    drawPreviewComposition(ctx, x, y, drawWidth, drawHeight);
  };

  async function drawOnCanvasForProcessing(baseImage) {
    ...
    const maxSide = parseInt(maxOutputSizeSlider.value, 10);
    const scaled = getScaledDimensions(targetWidth, targetHeight, maxSide);
    ...
    ctx.drawImage(baseImage, 0, 0, scaled.width, scaled.height);

    if (preloadedTemplate) {
      ctx.globalAlpha = templateOpacitySlider.value / 100;
      ctx.drawImage(preloadedTemplate, 0, 0, scaled.width, scaled.height);
      ctx.globalAlpha = 1;
    }

    if (preloadedLogo) {
      const scale = logoSizeSlider.value / 100;
      const newW = scaled.width * scale;
      const newH = preloadedLogo.height * (newW / preloadedLogo.width);
      const margin = Math.max(12, Math.round(scaled.width * 0.015));

      const positions = {
        'Top-Left': [margin, margin],
        'Top-Right': [scaled.width - newW - margin, margin],
        'Bottom-Left': [margin, scaled.height - newH - margin],
        'Bottom-Right': [scaled.width - newW - margin, scaled.height - newH - margin],
        'Center': [(scaled.width - newW) / 2, (scaled.height - newH) / 2]
      };

      const [x, y] = positions[logoPositionSelect.value];
      ctx.globalAlpha = logoOpacitySlider.value / 100;
      ctx.drawImage(preloadedLogo, Math.floor(x), Math.floor(y), Math.floor(newW), Math.floor(newH));
      ctx.globalAlpha = 1;
    }
  ...
  async function buildBlobFromCanvas(canvas) {
    ...
    try {
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        try {
          ...
          workingCanvas = await drawOnCanvasForProcessing(baseImage);
          const blob = await buildBlobFromCanvas(workingCanvas);
          ...
          if (isIOS()) {
            ...
            note = 'على أندرويد: إذا لم يعمل زر التنزيل في صورة ما، جرّب الضغط المطول على المعاينة.';
          }
          ...
          console.error('Image processing failed:', file.name, itemError);

          const failedName = file.name || `image-${i + 1}`;
          const li = document.createElement('li');
          li.innerHTML = `
            <div class="result-name">${failedName}</div>
            <div class="result-note">تعذر معالجة هذه الصورة تحديدًا. جرّب تقليل دقتها أو اختيار صورة أخرى.</div>
          `;
          resultsList.appendChild(li);
        } finally {
          if (workingCanvas) releaseCanvas(workingCanvas);
      ...
      if (postProcessMessage) {
        ...
        postProcessMessage.textContent = 'اكتملت معالجة الصور المتاحة داخل التطبيق.';
      }
    } catch (error) {
      console.error(error);
      if (progressContainer) progressContainer.style.display = 'none';
      if (postProcessMessage) {
        postProcessMessage.style.display = 'block';
        postProcessMessage.textContent = 'حدثت مشكلة عامة أثناء معالجة الصور.';
      }
      alert('حدثت مشكلة عامة أثناء معالجة الصور.');
    } finally {
  ...
  function saveSettings() {
    const settings = {
      ...
      maxOutputSize: maxOutputSizeSlider.value
    };
  ...
  function loadSettings() {
    ...
    maxOutputSizeSlider.value = settings.maxOutputSize || 2200;
  ...
  function resetAll() {
    ...
    maxOutputSizeSlider.value = 2200;
